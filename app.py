...
def show_searchbar_trailer_intro():
    """Shows the search bar area with a black, glowing/blinking shlok trailer effect for ~12s,
    with a deep slow recitation, then reveals the normal search bar."""
    mantra_display = (
    ...
    with placeholder.container():
        st.markdown(
            ...
            .svag-trailer-text {{
                ...
                font-size: 15px;
                line-height: 1.9;
                text-shadow: 0 0 10px rgba(201,162,39,0.6), 0 0 20px rgba(76,175,110,0.4);
                animation: svag-trailer-flicker 2s ease-in-out infinite;
            }}
            @keyframes svag-trailer-flicker {{
                0%, 100% {{ opacity: 0.5; }}
                50%      {{ opacity: 1; }}
            }}
            </style>
            <div class="svag-trailer-box">
                <div class="svag-trailer-text">{mantra_display}</div>
            </div>
            """,
        ...
        if mantra_audio:
            ...
            components.html(
                f"""
                <audio autoplay>
                    <source src="data:audio/mp3;base64,{mantra_b64}" type="audio/mp3">
                </audio>
                """,
                height=0,
            )
