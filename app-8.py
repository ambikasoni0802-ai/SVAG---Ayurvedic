...
def show_searchbar_trailer_intro():
    """Non-blocking cinematic overlay: dramatic movie-title-style shlok fades in/out over the
    black background while the app (logo, icons, search bar) stays visible and usable underneath.
    Plays a deep, slightly echoing male recitation once."""
    mantra_display = (
    ...
    st.markdown(
        ...
        .svag-cinematic-overlay {{
            ...
            animation: svag-overlay-lifecycle 12s ease-in-out forwards;
        }}
        .svag-cinematic-text {{
            ...
            animation: svag-text-flicker 1.8s ease-in-out infinite;
        }}
    ...
    if mantra_audio:
        mantra_b64 = base64.b64encode(mantra_audio).decode()
        # A second, quieter, slightly delayed copy creates a subtle echo ("gunj") effect
        components.html(
            f"""
            <audio id="svag-mantra-main" autoplay></audio>
            <audio id="svag-mantra-echo"></audio>
            <script>
            const mainAudio = document.getElementById('svag-mantra-main');
            const echoAudio = document.getElementById('svag-mantra-echo');
            mainAudio.src = "data:audio/mp3;base64,{mantra_b64}";
            echoAudio.src = "data:audio/mp3;base64,{mantra_b64}";
            echoAudio.volume = 0.18;
            mainAudio.play().catch(function(e) {{}});
            setTimeout(function() {{
                echoAudio.play().catch(function(e) {{}});
            }}, 220);
            </script>
            """,
            height=0,
...
st.markdown(
    ...
    /* Cinematic black theme across the whole app — UI stays visible on top */
    .stApp {
        background: radial-gradient(ellipse at center, #12140f 0%, #000000 100%) !important;
        color: #EAF2EC;
    }
