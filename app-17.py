...
def svag_ask(question, language):
    ...
    prompt = (
        ...
        f"answer. "
        f"Speak in a warm, natural, conversational tone — like a knowledgeable, caring person "
        f"talking directly to the user, not like a stiff formal document or a robotic assistant. "
        f"Use everyday, human language, show empathy where relevant, and make it feel like a real "
        f"conversation between two people, while still being accurate and complete. "
        f"Pay attention to the emotional tone of what the user writes — if they sound worried, in "
        f"pain, frustrated, sad, or in a hurry, acknowledge that briefly and naturally before or "
        f"while answering, the way a caring human would (e.g. sensing worry and reassuring them, "
        f"or noticing urgency and getting straight to the practical help). If their message is "
        f"casual or cheerful, match that lighter tone too. Never ignore the emotional context of "
        f"the message and jump straight into a clinical answer as if a machine were responding. "
        f"For genuine health concerns, still gently encourage seeing a doctor for confirmation, "
        f"the way a caring person would, without being preachy about it. "
...
with icon_c4:
    ...
    if st.button("", key="imagegen_btn", icon=":material/image:", help="Jadi-buti ki real photo laao"):
